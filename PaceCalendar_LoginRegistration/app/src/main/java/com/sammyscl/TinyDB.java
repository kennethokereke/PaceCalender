...
public class TinyDB {
    ...
    /**
     * Put HashMapListIntegerInteger of Integer into SharedPreferences with 'key' and save
     *
     * @param key     SharedPreferences key
     * @param hashMapListIntegerInteger HashMapListIntegerInteger of Integer to be added
     */
    public void putHashMapListIntegerInteger(String key, HashMap<Integer, Integer> hashMapListIntegerInteger) {

        for (Map.Entry entry : hashMapListIntegerInteger.entrySet())
            preferences.edit().putString(entry.getKey().toString(), entry.getValue().toString()).apply();
    }


    /**
     * Get parsed HashMap of Integers from SharedPreferences at 'key'
     *
     * @param key SharedPreferences key
     * @return HashMap of Integers
     */
    public HashMap<Integer, Integer> getHashMapListIntegerInteger(String key) {

        HashMap<Integer, Integer> hashMapListIntegerInteger = new HashMap<>();

        for (Map.Entry entry : preferences.getAll().entrySet())
            hashMapListIntegerInteger.put(Integer.parseInt((String)entry.getKey()), Integer.parseInt((String)entry.getValue()));

        return hashMapListIntegerInteger;
    }
...
//    }
    ...
    public Date getDate(final String key, final Date defValue) {
        if (!preferences.contains(key + "_value") || !preferences.contains(key + "_zone")) {
            return defValue;
        }
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeInMillis(preferences.getLong(key + "_value", 0));
        calendar.setTimeZone(TimeZone.getTimeZone(preferences.getString(key + "_zone", TimeZone.getDefault().getID())));
        return calendar.getTime();
    }

    public void putDate(final String key, final Date date, final TimeZone zone) {
        preferences.edit().putLong(key + "_value", date.getTime()).apply();
        preferences.edit().putString(key + "_zone", zone.getID()).apply();
    }
}
