...
import com.sammyscl.calendar.adapters.MyWeekPagerAdapter
import com.sammyscl.calendar.extensions.config
import com.sammyscl.calendar.extensions.seconds
import com.sammyscl.calendar.helpers.Formatter
import com.sammyscl.calendar.helpers.WEEK_START_DATE_TIME
import com.sammyscl.calendar.interfaces.WeekFragmentListener
import com.sammyscl.calendar.views.MyScrollView
...
class WeekFragmentsHolder : MyFragmentHolder(), WeekFragmentListener {
            ...
            addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                ...
                override fun onPageSelected(position: Int) {
                    ...
                    if (isGoToTodayVisible != shouldGoToTodayBeVisible) {
                        (activity as? MainActivityCal)?.toggleGoToTodayVisibility(shouldGoToTodayBeVisible)
    ...
    private fun setupWeeklyActionbarTitle(timestamp: Int) {
        ...
        if (startDateTime.monthOfYear == endDateTime.monthOfYear) {
            ...
            (activity as? MainActivityCal)?.supportActionBar?.title = newTitle
        } else {
            val endMonthName = Formatter.getMonthName(context!!, endDateTime.monthOfYear)
            (activity as? MainActivityCal)?.supportActionBar?.title = "$startMonthName - $endMonthName"
        }
        (activity as? MainActivityCal)?.supportActionBar?.subtitle = "${getString(R.string.week)} ${startDateTime.plusDays(3).weekOfWeekyear}"
    }
